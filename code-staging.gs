...
function registrarSalidaInterna(datos) {
  // v19/09/2026 — BLINDAJE: esta función nunca tuvo candado ni protección
  // anti-duplicado, a diferencia de procesarVenta (v10.5) e
  // ingresarMercaderia (v10.11/v10.0). Causa real reportada: con conexión
  // lenta, el botón "Confirmar salida" volvía a habilitarse tras el
  // timeout, y un segundo toque duplicaba la salida (descuenta stock dos
  // veces, dos filas en SALIDAS). Mismo patrón que las otras dos: candado
  // incondicional + idOperacion (columna K de SALIDAS, nueva) chequeado
  // antes de tocar stock.
  var lockSalida = LockService.getScriptLock();
  try { lockSalida.waitLock(15000); } catch (lockErrSalida) {
    return { success: false, mensaje: 'El sistema está procesando otra salida. Esperá unos segundos e intentá de nuevo.' };
  }
  try {
  try {
    var ss = SpreadsheetApp.openById(SS_ID);
    ...
    if (!shSal) {
      shSal = ss.insertSheet('SALIDAS');
      shSal.getRange(1, 1, 1, 11).setValues([[
        'Fecha', 'Hora', 'Producto', 'ID', 'Cantidad', 'Costo Unit.', 'Precio Venta', 'Motivo', 'Observación', 'Vendedor', 'idOperacion'
      ]]);
      shSal.getRange(1, 1, 1, 11).setFontWeight('bold');
    }

    // Idempotencia: mismo criterio que ingresarMercaderia — si este
    // idOperacion ya está en columna K, la salida ya se registró; no
    // volver a descontar stock ni a agregar otra fila.
    var idOpSalida = String(datos.idOperacion || '').trim();
    if (idOpSalida) {
      var ultimaFilaSal = shSal.getLastRow();
      if (ultimaFilaSal >= 2) {
        var rangoK = shSal.getRange(2, 11, ultimaFilaSal - 1, 1);
        var encontradaSal = rangoK.createTextFinder(idOpSalida).matchEntireCell(true).findNext();
        if (encontradaSal) {
          var filaExistente = shSal.getRange(encontradaSal.getRow(), 1, 1, 11).getValues()[0];
          return {
            success: true,
            yaExistia: true,
            nuevoStock: null,
            mensaje: 'Esta salida ya se había registrado antes — no se descontó stock de nuevo.'
          };
        }
      }
    }
